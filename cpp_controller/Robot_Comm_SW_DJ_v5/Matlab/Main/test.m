...
%%
fname = 'Data_Speedl_F_53_P_40.00_I_30.00_D_1.00.txt';
...
F0 = F_ctrl(1,3);
F_norm = F0 - F_ctrl(:,3);
Target_F_norm = F0 - Target_F;

%%

% 시간영역 지표
[Mp, tr, ts, ess] = fun_timeDomainMetrics(t_ctrl,F_norm,Target_F_norm);
...
% 스펙트럼 분석
[f, Pxx] = fun_computeFFTAnalysis(t_ctrl,F_norm);
%%
% 결과 출력
...
Fz_RMSE = rmse(Target_F_norm,F_norm);
...
plot(t_ctrl,F_norm);
hold on;
yline(Target_F_norm,'r--','LineWidth',3);
xlim([0 t_ctrl(end)]);
xlabel('Time (s)');
