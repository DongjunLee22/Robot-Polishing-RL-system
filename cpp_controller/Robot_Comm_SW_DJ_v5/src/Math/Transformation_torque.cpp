#include "Transformation_torque.h"

Transformation_Torque::Transformation_Torque(float T_Biased[3], float pos_actual[6]) {
    // 바이어스된 토크 벡터를 센서 기준으로 설정
    T_Biased_wrt_sensor[0] = T_Biased[0];
    T_Biased_wrt_sensor[1] = T_Biased[1];
    T_Biased_wrt_sensor[2] = T_Biased[2];
...
void Transformation_Torque::calculateRotationMatrix(float ZYZ_a_rad, float ZYZ_b_rad, float ZYZ_c_rad) {
    // 각 축에 대한 회전 행렬 계산
    float Rz_alpha[3][3] = {
...
void Transformation_Torque::transformTorque(float T_wrt_base[3]) {
	// 센서 기준의 바이어스된 토크 벡터를 ZYZ 회전 행렬을 사용하여 변환
    for (int i = 0; i < 3; ++i) {
        T_wrt_base[i] = 0;
        for (int j = 0; j < 3; ++j) {
            T_wrt_base[i] += R_zyz[i][j] * T_Biased_wrt_sensor[j];
        }
    }
}
