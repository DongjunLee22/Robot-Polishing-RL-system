...
void COScopeCtrl::SetRanges(double dLower, double dUpper)
{
	...
	InvalidateCtrl();  // 화면을 갱신하여 변경된 설정이 반영되도록 함
}

void COScopeCtrl::SetRanges_pos(double dLower, double dUpper)
{
	ASSERT(dUpper > dLower);
...
void COScopeCtrl::AppendPoints(float dNewPoint[], bool bInvalidate, bool bAdd2List, bool bUseTrendRatio)
{
	...
	for (iTrend = 0; iTrend < m_NTrends; iTrend++)
	{
		...
		if (bUseTrendRatio)
			m_PlotData[iTrend].dCurrentPosition = (float)dNewPoint[iTrend] / m_PlotData[iTrend].iTrendRatio;
		else
...
int COScopeCtrl::ReCreateGraph()
{
	...
	float* pAddPoints = new float[m_NTrends];
	...
	{
		for (int iTrend = 0; iTrend < m_NTrends; iTrend++)
			pAddPoints[iTrend] = (float)m_PlotData[iTrend].lstPoints.GetNext(pPosArray[iTrend]);
...
void COScopeCtrl::SetYGridSpacing(double spacing)
{
	//m_YGridSpacing = spacing;
	//m_nYGrids = static_cast<int>((m_PlotData[0].dUpperLimit - m_PlotData[0].dLowerLimit) / spacing);
	//InvalidateCtrl();  // 화면을 갱신하여 변경된 설정이 반영되도록 함
	...
	// 범위 계산
	double range = m_PlotData[0].dUpperLimit - m_PlotData[0].dLowerLimit;

	// 정확한 그리드 수 계산
	m_nYGrids = static_cast<int>(std::round(range / m_YGridSpacing));

	// 간격이 정수로 나누어떨어지지 않을 경우 스페이싱 재조정
	if (m_nYGrids > 0)
	{
		m_YGridSpacing = range / static_cast<double>(m_nYGrids);
	}
